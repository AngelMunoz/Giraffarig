...
let configureApp (app: IApplicationBuilder) =
    ...
    (match env.EnvironmentName with
     ...
     | _ -> app.UseGiraffeErrorHandler errorHandler).UseAuthentication().UseHttpsRedirection().UseCors(configureCors)
        .UseStaticFiles().UseGiraffe(webApp)
...
let configureServices (services: IServiceCollection) =
    services.AddCors().AddGiraffe().AddAuthentication(authenticationOptions)
...
let main _ =
    ...
    WebHostBuilder().UseKestrel().UseContentRoot(contentRoot).UseIISIntegration()
