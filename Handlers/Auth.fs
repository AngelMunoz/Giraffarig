...
let login: HttpHandler =
    fun (next : HttpFunc) (ctx : HttpContext) -> task {
        ...
        let maybe = 
                UserCollection.Find(fun user -> user.Email = payload.Email)
                    .ToEnumerable() 
                |>  Seq.tryHead
        return! match maybe with
                ...
                | Some user -> 
                    ...
                    | true -> Successful.OK ({ Token = issueJWT user; User = userToDTO user; }) next ctx
    }
...
let signup: HttpHandler =
    fun (next : HttpFunc) (ctx : HttpContext) -> task {
        ...
        with
        | :? Exception -> return! ServerErrors.INTERNAL_ERROR "Something Went Wrong" next ctx
    }